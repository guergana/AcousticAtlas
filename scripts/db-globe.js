  ...
  firebase.initializeApp({
    ...
    //storageBucket: 'acoustic-atlas.appspot.com'
     ...
     // Globe
    Cesium.Ion.defaultAccessToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiJhMzE4YzhlMC0zYWMyLTQyNWItYmFkZi04MzJhYzEyNGUxODAiLCJpZCI6MTY5MDUsInNjb3BlcyI6WyJhc3IiLCJnYyJdLCJpYXQiOjE1NzEzMDUzNTl9.H2-dtkBwj9ik_vvYrE0deVJKfEWaJXz0YkM0U1QCHss';
 ...
 function img_create(src, title) {
	 ...
	 img.src = '/img/' + src;
	 if ( title != null ) {img.alt = title;img.title = title;} 
	 return img;
}
...
dataSourcePromise.then(function(dataSource) {
    ...
    for (var i = 0; i < singleDigitPins.length; ++i) {
        singleDigitPins[i] = pinBuilder.fromText('' + (i + 2), Cesium.Color.GRAY, 36).toDataURL();
    }
			console.log('outside customStyle');
...
//end of clusters
   ...
   var handler = new Cesium.ScreenSpaceEventHandler(viewer.scene.canvas);
    ...
    handler.setInputAction(function(movement) {
        ...
        if (Cesium.defined(pickedEntity) && Cesium.defined(pickedEntity.billboard)) {
            ...
            pickedEntity.billboard.color = Cesium.Color.YELLOW;
  ...
  handler.setInputAction(function(e) {
    ...
    var cartesian = new Cesium.Cartesian3(pickedEntity.position._value.x, pickedEntity.position._value.y, pickedEntity.position._value.z );
    if (cartesian) {
        ...
        var result = id + '- '+  name + ': '+ longitudeString + ', '+ latitudeString;  // We can reuse this 
		...
		docRef.get().then(function(doc) {
			if (doc.exists) {
				...
				let descriptionContainer = document.querySelectorAll("#description-container span")[0];
				...
				// if(imageContainer.children.length > 1){
					// imageContainer.children[1].remove();
				// }
				//imageContainer.appendChild(image);
				// let panelMain = document.getElementsByClassName("panel-main")[0];
				// panelMain.appendChild(imageContainer);
		...
		//clicked on a cluster
		  var pickedLabel = viewer.scene.pick(e.position);
